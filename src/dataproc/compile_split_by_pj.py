"""
    ...
    Split into train/dev/test (*BY PART-WHOLE*) with given split percentages
"""
...
if __name__ == '__main__':
    ...
    pj2rest = defaultdict(list)
    for ex in examples:
        pj2rest[tuple(ex[1:3])].append(ex[:1] + ex[3:])
    pjs = list(set(pj2rest.keys()))
    
    random.shuffle(pjs)
    for i,pj in enumerate(pjs):
        rank = i / len(pjs)
        if rank < train_sz:
            for rest in pj2rest[pj]:
                example = rest[:1] + list(pj) + rest[1:]
